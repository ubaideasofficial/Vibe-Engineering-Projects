...
**Allowed in addition to the photo library:**
- ✅ Hand-authored **inline SVG** — icons, diagrams, wireframe pipelines, the exploded-weld NDT illustration, arrows, dividers, patterns, noise/grain textures, gradient meshes, the brushed-metal texture. Vector/CSS art is *encouraged*, because it costs no assets and scales perfectly.
- ✅ CSS gradients, duotone/blend-mode treatments, blurs, masks applied **to the supplied photos**.
- ✅ The supplied photo reused across multiple sections with different crops/treatments.

**If a section has no suitable photo:** do NOT source one. Instead solve it with (a) inline SVG art, (b) a typographic/color-block composition, (c) reuse an existing photo with a different duotone treatment, or (d) leave it image-free — Minimalist and Brutalist sections need no photography at all.
